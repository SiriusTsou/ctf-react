...
module.exports=async function({getNamedAccounts, ethers, deployments, ethernal}) {
  ...
  const ret = await deploy( 'CaptureTheFlag', {gasPrice, from: deployer, args: [forwarder]} )
  await ethernal.push({
    name: "CaptureTheFlag",
    address: ret.address,
  });
  ...
  if ( process.env.DEPLOY_PM || chainId == 1337) {
    console.log('Deploying paymaster:')
    const ret = await deploy('SingleRecipientPaymaster', { gasPrice, from: deployer, args: [ctf.address] })
    await ethernal.push({
      name: "SingleRecipientPaymaster",
      address: ret.address,
    });
